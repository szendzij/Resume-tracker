      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">Masowa zmiana daty wysłania</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Zaktualizuj datę dla {selectedCount} zaznaczonych ofert</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleApply} className="p-6 space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-xs font-medium border bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer"
              >
                Wczoraj
              </button>
              <button
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 rounded-xl p-3 border border-slate-200/80 dark:border-slate-700 text-xs">
            ...
            <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
              {selectedApps.slice(0, 5).map((app) => (
                <span
                  key={app.id}
                  className="px-2 py-0.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-[11px] text-slate-600 dark:text-slate-300 truncate max-w-[200px]"
                >
                  {app.company} – {app.role}
                </span>
              ))}
              {selectedApps.length > 5 && (
                <span className="px-2 py-0.5 bg-slate-200/60 dark:bg-slate-700 rounded text-[11px] text-slate-500 dark:text-slate-400 font-medium">
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100 dark:border-slate-800">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
            >
              Anuluj
            </button>
            <button
